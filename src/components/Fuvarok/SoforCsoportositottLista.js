...
import { PiCaretLeftLight, PiCaretRightLight, PiCalendarBlankLight } from "react-icons/pi";
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-ink-100 bg-white p-8 text-center shadow-soft dark:border-ink-800 dark:bg-ink-900">
          <PiCalendarBlankLight className="h-8 w-8 text-ink-300" />
          <p className="text-sm text-ink-400">Nincs fuvar ezen a héten.</p>
          <button
            type="button"
            onClick={() => lepHetet(-1)}
            className="rounded-full bg-slate-100 px-4 py-2 text-xs font-bold uppercase tracking-wide text-ink-600 hover:bg-slate-200 dark:bg-ink-800 dark:text-ink-300 dark:hover:bg-ink-700"
          >
            Előző hét megnyitása
          </button>
        </div>
      ) : (
              ...
              {csoport.fuvarok.map((f) => (
                ...
                >
                  <span className="text-ink-600 dark:text-ink-300">
                    {formatFuvarDatum(f.lerakas_datuma) || "—"} ·{" "}
                    {f.felrako_ceg || f.lerako_ceg ? `${f.felrako_ceg || "—"} → ${f.lerako_ceg || "—"}` : "Nincs útvonal megadva"}
                  </span>
