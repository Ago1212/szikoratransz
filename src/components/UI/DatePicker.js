    ...
    <div className={`relative ${className}`} ref={wrapperRef}>
      ...
      {open &&
        ...
        createPortal(
          <div
            ref={panelRef}
            className="fixed z-[9999] rounded-2xl border border-ink-100 bg-white p-3 shadow-soft-xl dark:border-ink-700 dark:bg-ink-900"
          ...
          >
            <div className="mb-3 flex items-center justify-between px-1">
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-lg text-ink-400 transition-colors duration-150 hover:bg-slate-100 hover:text-ink-700 dark:hover:bg-ink-800 dark:hover:text-ink-100"
              >
                <PiCaretLeftLight className="h-4 w-4" />
              </button>
              <span className="text-sm font-semibold capitalize text-brand-900 dark:text-ink-50">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-[11px] font-semibold uppercase text-ink-400 dark:text-ink-500">
              {WEEKDAYS.map((d) => (
                <div key={d}>{d}</div>
              ))}
            </div>
                    ...
                    className={`flex h-11 w-full items-center justify-center rounded-lg text-sm transition-colors duration-150 ${
                      isSelected
                        ...
                        : isToday(day)
                          ...
                          : inMonth
                            ? "text-ink-700 hover:bg-slate-100 dark:text-ink-100 dark:hover:bg-ink-800"
                            : "text-ink-400 hover:bg-slate-50 dark:text-ink-600 dark:hover:bg-ink-800"
                    }`}
                  >
                    {format(day, "d")}
                  </button>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-ink-100 pt-3 dark:border-ink-700">
              <button
                type="button"
                onClick={() => {
                  emitChange(new Date());
                ...
                className="text-xs font-semibold text-brand-600 hover:text-brand-800 dark:text-brand-300 dark:hover:text-brand-100"
              >
                Ma
              </button>
              {!required && (
                <button
                  ...
                  className="text-xs font-medium text-ink-400 hover:text-ink-700 dark:text-ink-500 dark:hover:text-ink-200"
                >
                  Törlés
                </button>
              )}
