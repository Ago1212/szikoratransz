...
import { PiMagnifyingGlassLight, PiTruckLight, PiCheckCircleFill, PiClockLight } from "react-icons/pi";
...
export default function JarmuValaszto() {
  ...
  const loadPending = (soforId) => {
    fetchAction("getSajatJarmuValtasKerelmek", { sofor_id: soforId }).then((result) => {
      if (result?.success) {
        setPendingKerelem((result.kerelmek || []).find((k) => k.tipus === "kamion") || null);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    fetchAction("getKamionok", { id: userData.admin }).then((result) => {
  ...
  const filtered = useMemo(() => {
    return kamionok.filter((k) => {
      if (filter !== "mind" && k.allapot !== filter) return false;
      if (!search.trim()) return true;
      const term = search.trim().toLowerCase();
      return (
        (k.rendszam || "").toLowerCase().includes(term) ||
        (k.tipus || "").toLowerCase().includes(term)
      );
    });
  }, [kamionok, filter, search]);

  const request = async (kamion) => {
    if (String(kamion.id) === String(user?.kamion)) return;
    setSaving(true);
    ...
    setSaving(false);
    if (result?.success) {
      toast.success(`Kérés elküldve: ${kamion.rendszam} — várj az admin jóváhagyására.`);
      loadPending(user.id);
    } else {
  ...
  const handleQr = (value) => {
    const match = kamionok.find(
      (k) => (k.rendszam || "").toLowerCase().replace(/\s/g, "") === value.toLowerCase().replace(/\s/g, ""),
    );
    if (match) {
      request(match);
    } else {
      toast.error(`Nincs "${value}" rendszámú kamion.`);
    }
  };

  return (
    <div className="flex flex-col gap-3">
      <MobileHeader title="Kamion kiválasztása" />
      ...
      <div className="flex items-center gap-2">
        ...
        <QrScanButton onResult={handleQr} label="Kamion QR-kód beolvasása" />
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-ink-100 bg-white p-6 text-center text-sm text-ink-400 shadow-soft">
          Nincs a szűrésnek megfelelő kamion.
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {filtered.map((k) => {
            const active = String(k.id) === String(user?.kamion);
            const requested = String(k.id) === String(pendingKerelem?.jarmu_id);
            return (
              <button
                key={k.id}
                type="button"
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block font-mono text-sm font-bold text-ink-900">{k.rendszam}</span>
                  <span className="block truncate text-xs text-ink-500">
                    {k.tipus || "—"}
                    {k.aktualis_km ? ` · ${Number(k.aktualis_km).toLocaleString("hu-HU")} km` : ""}
                  </span>
                </span>
                {active ? (
                  ...
                  <span
                    className={`flex-shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ${
                      ALLAPOT_TONE[k.allapot] || "bg-ink-100 text-ink-500"
                    }`}
                  >
                    {ALLAPOT_LABEL[k.allapot] || k.allapot}
                  </span>
                )}
              </button>
            );
